...
def get_scatter_plot(merged_sim, merged_cor, code, name, log):
    ...
    if log == True:
        layout = go.Layout(title = "Scatter Plot (Log Scale) <br> {0} - {1}".format(code.upper(), name),
                       xaxis = dict(title = 'Simulated Water Level (cm)', type = 'log', ), 
                       yaxis = dict(title = 'Observed Water Level (cm)', type = 'log', autorange = True), 
                       showlegend=True)
    else:
        layout = go.Layout(title = "Scatter Plot <br> {0} - {1}".format(code.upper(), name),
                       xaxis = dict(title = 'Simulated Water Level (cm)',  ), 
                       yaxis = dict(title = 'Observed Water Level (cm)', autorange = True), 
                       showlegend=True)
    # Plotting data
...
@controller(name='get_stations',url='national-water-level-forecast-brazil/get-stations')
...
@controller(name='get_data',url='national-water-level-forecast-brazil/get-data')
def get_data(request):
    ...
    #station_code = "10075000"
    #station_comid = "9038269"
    #station_name = "TAMISHIYACU"
    #plot_width = 1000
    #plot_width_2 = 0.5*plot_width
    ...
    return render(request, 'national_water_level_forecast_brazil/panel.html', context)





@controller(name='get_metrics_custom',url='national-water-level-forecast-brazil/get-metrics-custom')
...
@controller(name='get_raw_forecast_date',url='national-water-level-forecast-brazil/get-raw-forecast-date')
